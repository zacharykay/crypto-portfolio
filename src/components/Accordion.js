...
const Accordion = ({
  ...
  return (
    <Wrapper>
      ...
      >
        <CryptoCard
          currentAssetValue={amounts ? currentAssetValue : 0}
          assetPercentChange={amounts ? assetPercentChange : 0}
          {...props}
        />
        {accordionVisible && (
          <React.Fragment>
              ...
              <div className="card-data">
                <p className="investment-info">
                  ...
                  <span className="lowpacity">
                    / {convertUSDToCrypto(amounts[id], null, prices[id])}{" "}
                    {symbol.toUpperCase()}
                    &nbsp;{" | "}&nbsp;${prices[id]}
                    {" | "}
                    {(currentAssetValue() / investment * 100).toFixed(1)}%
                  </span>
                </p>
              </div>
            )}
